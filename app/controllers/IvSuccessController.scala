...
class IvSuccessController @Inject() (
      ...
      case Some(userAnswers) =>
        userAnswers.get(UTRPage).map { utr =>
          lazy val onRelationshipNotFound =
            Future.successful(Redirect(routes.IsAgentManagingEstateController.onPageLoad(NormalMode)))
  ...
  private def onRelationshipFound(utr: String, userAnswers: UserAnswers)(implicit
